...
The ~__repo_config~ repository is created with the post-update hook configured (instead of the update hook, which is called *before* packs are received), so that all updates to the ~master~ branch would trigger ~aegis simple-mode aegis-sync~. This command would go through the repository and ensure all the repositories are created.
...
the update of this ~aegis_sync~ cannot be done during the execution of the ~update~ hook since the update (to refs/heads/master) is only complete *after* the hook has completed running. we prepare the command ~aegis simple-mode aegis-sync~ for this.
