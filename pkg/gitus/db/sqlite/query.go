...
`, pfx))
	...
	if err == sql.ErrNoRows { return nil, db.NewGitusDatabaseError(db.ENTITY_NOT_FOUND, "") }
	if err != nil { return nil, err }
...
`, pfx))
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, db.NewGitusDatabaseError(db.ENTITY_NOT_FOUND, "Cannot find key")
		}
		return nil, err
	}
	kt := ""
...
`, pfx))
	...
	if err == nil {
		return db.NewGitusDatabaseError(db.ENTITY_ALREADY_EXISTS, "Key already exists with the same name")
	}
...
func (dbif *SqliteGitusDatabaseInterface) CreateRepository(ns string, name string) (*model.Repository, error) {
	pfx := dbif.config.DatabaseTablePrefix
	fullName := ns + ":" + name
	tx, err := dbif.connection.Begin()
	if err != nil { return nil, err }
...
`, pfx))
	...
	err = v.Scan(&rowid)
	if err != nil { return err }
	if len(rowid) <= 0 { return db.NewGitusDatabaseError(db.ENTITY_NOT_FOUND, fmt.Sprintf("%s not found in %s", name, ns)) }
	tx, err := dbif.connection.Begin()
	if err != nil { tx.Rollback(); return err }
	stmt2, err := tx.Prepare(fmt.Sprintf(`
UPDATE %srepository
...
func (dbif *SqliteGitusDatabaseInterface) MoveRepository(oldNs string, oldName string, newNs string, newName string) error {
...
`, pfx))
	...
	var s string
	v.Scan(&s)
	if len(s) > 0 { return db.NewGitusDatabaseError(db.ENTITY_ALREADY_EXISTS, fmt.Sprintf("%s:%s already exists", newNs, newName)) }
