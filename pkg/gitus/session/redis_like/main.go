...
func (ssif *GitusRedisLikeSessionStore) RegisterSession(name string, session string) error {
...
func (ssif *GitusRedisLikeSessionStore) VerifySession(name string, target string) (bool, error) {
	key := fmt.Sprintf("%s:%s:session", ssif.config.Session.TablePrefix, name)
