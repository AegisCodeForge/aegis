...
// handles:
// aegis simple-mode keys-update $newrev
// aegis simple-mode config-update $newrev
// aegis simple-mode aegis-sync $path

func syncSimpleModeCache(ctx *routes.RouterContext) error {
	aegisSyncDir := path.Join(ctx.Config.GitRoot, "__aegis", "__repo_config", "aegis_sync")
	if ctx.SimpleModeConfigCache == nil {
		ctx.SimpleModeConfigCache = make(map[string]*model.SimpleModeNamespaceConfig, 0)
	}
	lst, err := os.ReadDir(aegisSyncDir)
	...
	if !ctx.Config.UseNamespace {
		...
		for _, v := range lst {
			...
			repoName := v.Name()
			repoDir := path.Join(aegisSyncDir, repoName)
		...
		for _, v := range lst {
			...
			nsName := v.Name()
			nsDir := path.Join(aegisSyncDir, nsName)
...
func HandleSimpleMode(ctx *routes.RouterContext, cmd string, newRev string) {
	...
	case "keys-update":
		...
		nsName := ""
		if ctx.Config.UseNamespace { nsName = "__aegis" }
		repoName := "__keys"
	...
	case "aegis-sync":
		...
		err := cmd.Run()
		if err != nil {
			printGitError(fmt.Sprintf("Failed to run git pull at aegis_sync: %s; %s; %s", err, stderrBuf.String(), stdoutBuf.String()))
			return
		}
		err = syncSimpleModeCache(ctx)
		if err != nil {
			printGitError(fmt.Sprintf("Failed to run git pull at aegis_sync: %s", err))
			return
		}
